...
public class PersonRepositoryImpl implements PersonRepository{
    ...
    public PersonRepositoryImpl() {
        ...
        personList.add(new Person(1, "Mariusz",9, "Brak"));
        personList.add(new Person(2, "Tomek",8, "Brak"));
        personList.add(new Person(3, "Andrzej",7, "Brak"));
    }
...
//    }
    ...
    public Person addPerson(Person personToAdd) throws PersonExistsEx {
        for (int i = 0; i < personList.size(); i++) {
            ...
            if (person.getId() == personToAdd.getId()) {
                throw new PersonExistsEx(personToAdd.getId());
            } else if (person.getId() > personToAdd.getId()) {
                personList.add(i, personToAdd);
                return personToAdd;
            }
        }
        // Jeśli nie znaleziono osoby o większym ID, dodaj na końcu
